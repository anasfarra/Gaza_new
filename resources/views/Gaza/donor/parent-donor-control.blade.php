...
<head>
    ...
    <title>@yield('title', 'منصة إعمار غزة - لوحة تحكم المانح')</title>

    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700&display=swap" rel="stylesheet">

    <script src="{{ asset('js/tailwind.js') }}"></script>
    <script src="{{ asset('js/tailwind-config.js') }}"></script>
    <script src="{{ asset('js/DonorControl-Panel.js') }}"></script>

    <link rel="stylesheet" href="{{ asset('css/DonorControl-Panel.css') }}">
@yield('css')
</head>
    ...
    <div id="root">
        <div class="min-h-screen bg-neutral-50">
            ...
            <header
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex justify-between items-center h-20">
                        <a class="flex items-center gap-3 group" href="{{ route('Gaza.donor-index') }}">
            ...
            <nav class="hidden lg:flex items-center gap-8">
                ...
                @endphp
                            <a class="{{ request()->routeIs('Gaza.DonorControl-Panel') ? $activeClass : $inactiveClass }}"
                                href="{{ route('Gaza.DonorControl-Panel') }}">لوحة تحكم المانح</a>
                            <a class="{{ request()->routeIs('Gaza.donor-index') ? $activeClass : $inactiveClass }}"
                                href="{{ route('Gaza.donor-index') }}">الرئيسية</a>
                            <a class="{{ request()->routeIs('Gaza.donor-damage-map') ? $activeClass : $inactiveClass }}"

                                href="{{ route('Gaza.donor-damage-map') }}">خريطة الأضرار</a>
                            <a class="{{ request()->routeIs('Gaza.donor-projects') ? $activeClass : $inactiveClass }}"
                                href="{{ route('Gaza.donor-projects') }}">المشاريع</a>
                            <a class="{{ request()->routeIs('Gaza.donor-about') ? $activeClass : $inactiveClass }}"
                                href="{{ route('Gaza.donor-about') }}">من نحن</a>
                            <a class="{{ request()->routeIs('Gaza.donor-content') ? $activeClass : $inactiveClass }}"
                                href="{{ route('Gaza.donor-content') }}">اتصل بنا</a>
                        </nav>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-right">
                        ...
                        <div>
                            ...
                            <div class="flex flex-col gap-2 text-sm text-neutral-400">
                                <a href="{{ route('Gaza.donor-damage-map') }}" class="hover:text-white transition-colors">خريطة الأضرار</a>
                                <a href="{{ route('Gaza.marketplace') }}" class="hover:text-white transition-colors">سوق المواد</a>
